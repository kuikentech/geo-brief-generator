...
CITATION KEY MAP (stable, matches evidence_builder.py output):
  ...
  --- Track 1 Supplemental Sources (E4C Hackathon 2026) ---
  SRC-020: Our World in Data — Water & Sanitation Access Trends (CC BY 4.0)
  SRC-021: ESMAP Tracking SDG 7 — Energy Progress Report (CC BY 4.0, World Bank)
  SRC-022: FAO AQUASTAT — Kenya Water Resources Profile (non-commercial)
  SRC-023: UNICEF Data — Kenya WASH and Child Health Indicators (open)
  SRC-024: NASA Earthdata Portal — Earth Observation Data Access (public domain)
  SRC-025: IHME Global Burden of Disease — Kenya/East Africa (non-commercial)
  SRC-026: HDX — Humanitarian Data Exchange, Kenya Context (varies by dataset)
  SRC-027: FAO FAOSTAT — Kenya Food Security and Agricultural Land Use (non-commercial)
"""
import os
import json
import logging
...
\u26a0 This brief is AI-assisted. All technical claims and cost estimates require validation by qualified engineers with direct field knowledge of the target region before any implementation decision.""",
        "citation_keys": ["SRC-001", "SRC-002", "SRC-004", "SRC-008", "SRC-009", "SRC-010", "SRC-011", "SRC-013", "SRC-020", "SRC-021", "SRC-023", "SRC-024", "SRC-025"],
        "assumptions": [
...
The predominant livelihoods are agropastoralism and pastoralism, with cattle, camels, and goats as the primary assets. Communities are predominantly dispersed and semi-nomadic, following seasonal pasture availability. Population density averages 8.3 persons per km², with significant seasonal variation. FAO FAOSTAT data confirms Northern ASAL counties account for approximately 60% of Kenya's severely food-insecure population, with undernourishment prevalence reaching 28.5% nationally [SRC-027]. Humanitarian Data Exchange (HDX/OCHA) assessments classify Turkana and Marsabit at IPC Phase 3+ (Crisis), with 1.2 million people requiring humanitarian food assistance (2024) [SRC-026]. The administrative capitals — Lodwar (Turkana, population approximately 35,000) and Marsabit town (Marsabit, approximately 18,000) — serve as the primary service delivery hubs and are the most realistic anchor points for supply chains and technical support [SRC-007].
...
Kenya's Rural Electrification Authority has installed mini-grids in Lodwar and some larger market towns, but grid access in dispersed rural communities averages just 5.2% [SRC-008], confirming that off-grid solar solutions are not merely preferable but practically necessary for these contexts. The road network connecting communities to these service hubs is sparse and seasonally impassable, averaging 4.2 km of road per 1,000 km² [SRC-006].""",
        "citation_keys": ["SRC-006", "SRC-007", "SRC-008", "SRC-025", "SRC-026", "SRC-027"],
        "assumptions": [
...
Regulatory Context: Kenya's Water Act 2016 and the Water Services Regulatory Board (WASREB) require water service providers to be licensed entities. Community water user associations can register as water service providers. County Water Departments must be engaged for water rights allocation, particularly for borehole drilling [SRC-017]. Treatment systems must achieve WHO standards: E. coli = 0 CFU/100 mL; turbidity ≤ 4 NTU at point of use; free residual chlorine 0.2–0.5 mg/L if chlorination is used [SRC-016].""",
        "citation_keys": ["SRC-013", "SRC-016", "SRC-017", "SRC-021"],
        "assumptions": [
...
Extreme Weather Risk: The INFORM Risk Index rates northern Kenya counties at 6.2 out of 10 — Very High risk tier — driven primarily by drought risk, food insecurity, and conflict risk in border areas [SRC-005]. Flash flooding during intense rain events can damage surface infrastructure. Dust storms during the January–March dry season can reduce PV panel output by 15–25% if panels are not cleaned regularly, requiring weekly cleaning protocol integration into community maintenance training.""",
        "citation_keys": ["SRC-001", "SRC-002", "SRC-003", "SRC-004", "SRC-005", "SRC-012", "SRC-022", "SRC-024"],
        "assumptions": [
...
Electricity Infrastructure: Grid access in the study region averages just 5.2% of households [SRC-008]. Kenya's Rural Electrification Authority has concentrated mini-grid investment in county capitals and market towns. The vast majority of rural communities operate entirely without electricity. This infrastructure context strongly justifies solar as the primary energy source — there is no viable alternative for off-grid water treatment. According to ESMAP's Tracking SDG 7 report, Kenya has made significant strides in rural electrification (from 7% in 2010 to 47% in 2022), yet this progress has bypassed the most remote ASAL communities, which remain at the frontier of energy poverty and constitute the primary deployment target for this intervention [SRC-021].
...
Environmental Considerations: POSITIVE. Solar-powered systems displace diesel generator use common in some NGO-run water schemes, eliminating diesel procurement logistics and air quality concerns. Borehole extraction requires hydrological assessment to confirm aquifer recharge rates are compatible with extraction volumes — over-extraction is a documented risk in arid regions with limited recharge [SRC-003].""",
        "citation_keys": ["SRC-001", "SRC-003", "SRC-004", "SRC-013", "SRC-019", "SRC-020", "SRC-021"],
        "assumptions": [
...
Risk 5 — Conflict and Security [Likelihood: Medium | Impact: Medium-High]: Northern Kenya border areas experience inter-communal conflict and cross-border raiding that can disrupt operations, endanger staff, and damage infrastructure. Mitigation: Conduct conflict-sensitive analysis before site selection; avoid siting installations near inter-communal boundary zones; engage traditional leadership and county security apparatus in project design; ensure community ownership to reduce targeting risk [SRC-005].""",
        "citation_keys": ["SRC-001", "SRC-003", "SRC-004", "SRC-005", "SRC-011", "SRC-013", "SRC-016", "SRC-018", "SRC-022", "SRC-025"],
        "assumptions": [
...
TRACK 1 SUPPLEMENTAL SOURCES (E4C Hackathon 2026 — Open License):
[SRC-020] Our World in Data — Water & Sanitation Access Trends. Our World in Data / Global Change Data Lab (University of Oxford). CC BY 4.0. https://ourworldindata.org/water-access Accessed: 2026-04-14.
[SRC-021] ESMAP Tracking SDG 7: The Energy Progress Report. Energy Sector Management Assistance Program / World Bank. CC BY 4.0. https://trackingsdg7.esmap.org/ Accessed: 2026-04-14.
[SRC-022] FAO AQUASTAT — Kenya Water Resources Profile. Food and Agriculture Organization of the United Nations. https://www.fao.org/aquastat/en/ Accessed: 2026-04-14.
[SRC-023] UNICEF Data — Kenya WASH and Child Health Indicators. United Nations Children's Fund (UNICEF). https://data.unicef.org/ Accessed: 2026-04-14.
[SRC-024] NASA Earthdata Portal — Earth Observation Data Access. NASA Earth Science Data Systems. Public Domain. https://www.earthdata.nasa.gov/ Accessed: 2026-04-14.
[SRC-025] IHME Global Burden of Disease 2021 — Kenya and East Africa. Institute for Health Metrics and Evaluation. https://ghdx.healthdata.org/ Accessed: 2026-04-14.
[SRC-026] Humanitarian Data Exchange (HDX) — Kenya Crisis and Context Data. UN OCHA. https://data.humdata.org/ Accessed: 2026-04-14.
[SRC-027] FAO FAOSTAT — Kenya Food Security and Agricultural Land Use. Food and Agriculture Organization. https://www.fao.org/faostat/en/ Accessed: 2026-04-14.

SOURCES EXCLUDED DUE TO LICENSING RESTRICTIONS:
IEA International Energy Agency — AI use requires explicit licensing (scraping restricted).
ITU ICT Statistics — Systematic download and redistribution restricted.
OECD Data Explorer — Non-commercial only; AI-specific use requires review.
UN-Habitat Urban Indicators Database — Non-commercial; redistribution restricted.
Global Water Monitor (GWM) — License terms not clearly published; verification required.""",
        "citation_keys": ["SRC-001", "SRC-002", "SRC-003", "SRC-004", "SRC-005", "SRC-006", "SRC-007", "SRC-008", "SRC-009", "SRC-010", "SRC-011", "SRC-012", "SRC-013", "SRC-014", "SRC-015", "SRC-016", "SRC-017", "SRC-018", "SRC-019", "SRC-020", "SRC-021", "SRC-022", "SRC-023", "SRC-024", "SRC-025", "SRC-026", "SRC-027"],
        "assumptions": [],
        "limitations": [
